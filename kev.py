...
def convert_kev_to_txt(kev_file, txt_file):
    traceprinter_path = "C:\\Users\\djohnso3\\Desktop\\traceprinter.exe"  # Update this path to the actual location of traceprinter.exe
...
def main():
    ...
    input_files = []
    while True:
        input_file = input("Please enter the name of an input text or KEV file (or 'done' to finish): ")
    ...
    for input_file in input_files:
        if input_file.lower().endswith('.kev'):
            txt_file = input_file.replace('.kev', '_converted.txt')
            if convert_kev_to_txt(input_file, txt_file):
